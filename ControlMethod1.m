...
%     lastPall=0;
% end
if abs(Agc-AgcStart)>1 % Cdead*Prgen % 固定大小:1 % 区间大小:Cdead*Prgen
    % 新来了一条指令，需要更新初始状态
...
Vresp_ideal=PallStart*deadK3/T02;% 理想的响应死区速度
...
if Ts <= T01
    if Ts<=T02
        ...
        if Pdg-PallStart < fang*PallStart*deadK3 % 按2MW算  %死区按照1%算：PdgStart*0.01
            %若机组功率未达到响应死区范围外
            Pall_resp_ideal=PallStart+fang*Vresp_ideal*Ts;% 理想的联合功率
            ...
            Pall_resp_ideal=PallStart+(PallStart*deadK3+0.5)*fang;% 理想的联合功率
            BatPower=Pall_resp_ideal-Pdg;% 储能出力
            if BatSoc<ParaSOC(2)
                BatPower=min(BatPower,0);% 超下限，只能充不能放
                Socflag=1;% Soc维护标志
        ...
        Pall_resp_ideal=PallStart+(PallStart*deadK3+0.5*(Ts-T02))*fang;% 理想的联合功率
        BatPower=Pall_resp_ideal-Pdg;% 储能出力
        if BatSoc<ParaSOC(2)
            BatPower=min(BatPower,0);% 超下限，只能充不能放
            Socflag=1;% Soc维护标志
...
%                     Pall_adj_ideal=Agc_adj-fang*Cdead_Res*Prgen;% 达到下界
